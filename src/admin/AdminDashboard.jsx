...
const AdminDashboard = () => {
  ...
  const renderContent = useMemo(() => {
    switch (activeTab) {
      case "dashboard":
        return <DashboardContent />;
      case "products":
        return <ProductsPage />; // Use separate Products page
      default:
        return <DashboardContent />;
    }
  }, [activeTab]);
  ...
  return (
    <div className="flex min-h-screen bg-gray-900 text-white font-inter">
      <Sidebar activeTab={activeTab} setActiveTab={setActiveTab} />
      <div className="flex-1 flex flex-col overflow-y-auto">
        <Topbar />
        <main className="flex-1 p-4 sm:p-6 lg:p-8 bg-gray-900">
          <h1 className="text-3xl font-bold mb-8 text-white capitalize">{activeTab}</h1>
          {renderContent}
        </main>
      </div>
    </div>
