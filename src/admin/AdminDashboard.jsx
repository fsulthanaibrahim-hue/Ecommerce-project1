      ...
      {/* 
         ...
         <div className="space-y-2 flex-1">
           ...
           <button
            ...
            className="flex items-center gap-3 px-4 py-2 text-red-400 hover:bg-red-500 hover:text-white rounded-lg transition"
           >
            <LogOut size={20} />
...
function DashboardContent({ products, users, orders }) {
  ...
  const stats = useMemo(() => {
    const totalOrders = Array.isArray(orders) ? orders.length : 8052;
    const totalRevenue = Array.isArray(orders) ? orders.reduce((s, o) => s + (Number(o.total) || 0), 0) : 620000;
    const newUsers = Array.isArray(users) ? users.length : 1300;
    const returns = 170;
    return {
      totalOrders,
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <StatsCard 
          title="New Users" 
          value={`${(stats.newUsers / 1000).toFixed(1)}K`} 
          ...
          <tbody>
            {products.map(p => (
              <tr key={p.id} className="border-b border-slate-700 hover:bg-slate-700/50">
                ...
                <td className="p-3">{p.stock > 0 ? "In Stock" : "Out"}</td>
                <td className="p-3">
    ...
    <div>
      ...
      <div className="space-y-3">
        ...
        {orders.map((o) => {
          // FIX: Correct Name
          const customerName =
            o.address?.name || o.shippingAddress?.name || o.userId;

          // FIX: Correct Date
          const orderDate = o.orderDate || o.date;
